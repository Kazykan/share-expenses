...
class Expense(models.Model):
    ...
    who_paid_user = models.ForeignKey(
        User,
        related_name="user_who_paid_expense",
        on_delete=models.CASCADE
    )
    place = models.ForeignKey(Place, on_delete=models.CASCADE)

    def __str__(self):
        return f'{self.name} {self.who_paid_user} - {self.cost} date: {self.date}'
...
class MoneyTransfer(models.Model):
    ...
    who_paid_user = models.ForeignKey(
        User,
        related_name="user_who_paid_money_transfer",
        on_delete=models.CASCADE
    )
    who_gets_user = models.ForeignKey(
        User,
        related_name="user_who_gets_money_transfer",
        on_delete=models.CASCADE
    )
    place = models.ForeignKey(Place, on_delete=models.CASCADE)
